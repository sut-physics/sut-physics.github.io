...
function startEditInfoCell(el) {
    ...
    // แสดงค่ามี comma ให้อ่านง่ายสำหรับ budget fields
    if (field === 'budget' || field === 'usedBudget' || field === 'remainingBudget') {
        input.value = parseNum(rawValue) ? parseNum(rawValue).toLocaleString('th-TH') : rawValue;
    } else {
    ...
    function finishEdit() {
        ...
        // ลบ comma ออกก่อนเก็บสำหรับ budget fields
        if (field === 'budget' || field === 'usedBudget' || field === 'remainingBudget') {
            newValue = String(parseNum(newValue));
        }
